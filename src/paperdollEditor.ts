...
export class PaperdollEditor {
  ...
  private async writeDraft(
    ...
    if (type === 'pd:apply' && this.latest.analysis.call) {
      ...
      const ok = await applyCheckedWorkspaceEdit(doc, edit, 'Paperdoll: update call');
      ...
      const at = Math.min(this.session.line, doc.lineCount - 1);
      if (lineInsideString(doc.getText(), at)) {
        void vscode.window.showWarningMessage('The cursor is inside a text — nothing was inserted.');
        return;
      }
      const pad = doc.lineAt(at).text.match(/^[ \t]*/)?.[0] ?? indent;
      // After the END of the statement at the cursor (it may span several lines).
      const line = doc.lineAt(Math.min(statementEndLine(doc.getText(), at), doc.lineCount - 1));
      const block = rendered.text
        .split('\n')
        .map((row, i) => (i === 0 ? `${pad}$ ${row}` : row))
        .join('\n');
      edit.insert(doc.uri, line.range.end, `\n${block}`);
      if (await applyCheckedWorkspaceEdit(doc, edit, 'Paperdoll: insert display')) {
        this.session = { ...this.session, line: line.lineNumber + 1, character: pad.length };
      }
      ...
      await applyCheckedWorkspaceEdit(doc, edit, 'Paperdoll: insert register');
      ...
      await applyCheckedWorkspaceEdit(doc, edit, 'Paperdoll: insert at cursor');
      ...
      const ok = await applyCheckedWorkspaceEdit(doc, edit, 'Paperdoll: optimize');
      if (ok) {
  ...
  /** The image fields the client script (webview/paperdollEditor.js) builds its form from. */
  static clientFields(): { field: string; label: string; group: 'head' | 'body' }[] {
